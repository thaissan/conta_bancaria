export const colors = { //reset: Restaura as cores padrões do Terminal;
    reset: "\x1b[0m",  
    fg: { //fg: Objeto Literal que contém as cores do texto (foreground);
        black: "\x1b[30m",
        red: "\x1b[31m",
        green: "\x1b[32m",
        yellow: "\x1b[33m",
        blue: "\x1b[34m",
        magenta: "\x1b[35m",
        cyan: "\x1b[36m",
        white: "\x1b[37m",
        gray: "\x1b[90m",
        crimson: "\x1b[38m",
        blackstrong : "\x1b[1;30m",
        redstrong : "\x1b[1;31m",
	    greenstrong : "\x1b[1;32m",
	    yellowstrong : "\x1b[1;33m", 
	    bluestrong : "\x1b[1;34m",
	    magentastrong : "\x1b[1;35m",
	    cyanstrong : "\x1b[1;36m",
        whitestrong: "\x1b[1;37m",
    },
    bg: { //bg: Objeto Literal que contém as cores do plano de fundo (background).
        black: "\x1b[40m",
        red: "\x1b[41m",
        green: "\x1b[42m",
        yellow: "\x1b[43m",
        blue: "\x1b[44m",
        magenta: "\x1b[45m",
        cyan: "\x1b[46m",
        white: "\x1b[47m",
        gray: "\x1b[100m",
        crimson: "\x1b[48m",
        blackbright: "\x1b[0;100m",
        redbright: "\x1b[0;101m",
        greenbright: "\x1b[0;102m",
        yellowbright: "\x1b[0;103m",
        bluebright: "\x1b[0;104m",
        magentabright: "\x1b[0;105m",
        cyanbright: "\x1b[0;106m",
        whitebright: "\x1b[0;107m"
    }
};